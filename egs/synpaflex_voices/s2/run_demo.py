...
def mod_acoustic_config(parser, merlin_path, exper_path, exper_type, d_mp_opts):
    ...
    if d_mp_opts['b_const_rate']:
        parser['Labels']['label_align'] = '%(TOPLEVEL)s/acoustic_model/data/label_phone_align'
...
if __name__ == '__main__':
    ...
    b_conv_labs_rate = 1 # Converts the phone aligned labels to variable rate if running in variable frame rate mode (d_mp_opts['b_const_rate'] = False)
    b_dur_train      = 1 # Merlin: Training of duration model.
    b_acous_train    = 1 # Merlin: Training of acoustic model.
    b_dur_syn        = 1 # Merlin: Generation of phone durations using the duration model.
    ...
    # PROCESS:===================================================================================================
    # Pre setup:-------------------------------------------------------------------------------
    exper_name  = 'nadine_magphase_%s_mag_dim_%s_phase_dim_%d_const_rate_%d' % (exper_type, d_mp_opts['mag_dim'], d_mp_opts['phase_dim'], d_mp_opts['b_const_rate'])
    ...
    # Setup Data:-----------------------------------------------------------------------------
    if b_setup_data:
        copytree(join(this_dir, 'nadine_' + exper_type + '_data', 'exper'), exper_path)
    ...
    if b_conv_labs_rate and not d_mp_opts['b_const_rate']: # NOTE: The script ./script/label_st_align_to_var_rate.py can be also called from comand line directly.
        label_phone_align = join(exper_path, 'acoustic_model', 'data', 'label_phone_align')
        label_phone_align_var_rate = pars_acous_train['Labels']['label_align']
        fs = int(pars_acous_train['Waveform']['samplerate'])
        ltvr.convert(file_id_list,label_phone_align, acoustic_feats_path, fs, label_phone_align_var_rate)
